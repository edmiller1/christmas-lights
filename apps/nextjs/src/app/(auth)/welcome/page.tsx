...
const WelcomePage = () => {
  ...
  const { data, refetch } = api.auth.getDatabaseSyncStatus.useQuery();

  useEffect(() => {
    const getDatabaseSyncStatus = async () => {
      try {
        await refetch();
      } catch {
        router.push("/sign-in");
  ...
  return (
    <div className="flex h-[90vh] w-full flex-1 items-center justify-center px-4">
